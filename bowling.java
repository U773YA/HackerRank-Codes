	...
	public static void main(String[] args)
	{
		...
		while(t-- > 0)
		{
			...
			for(int i = 0; i < s.length(); i++)
			{
				...
				if(ch == 'I')
				{
					//If I is found then check further - for continuous I's
					count++;
				}
				else
				{
					//Else if count not 0(when consecutive X's found) put in list and initialize to 0
					if(count != 0)
					{
						list.add(count);
					}
			...
			//do same for last heap [we can resolve this by explicitly adding a X at end]
			if(count > 0)
			{
				list.add(count);
			}
